...
function seg = region_seg(I,init_mask,max_its,alpha,display)
  ...
  %-- default value for parameter alpha is .1 if not provided
  if(~exist('alpha','var')) %--if not provided in function parameter
    alpha = .2; 
  end
  %-- enabling default behavior to display intermediate outputs
  if(~exist('display','var')) %--if not provided in function parameter
    display = true;
  end
  ...
  for its = 1:max_its   % Note: no automatic convergence test
    ...
    idx = find(phi <= 1.2 & phi >= -1.2);  %get the narrow band curve's
    
    %-- finding interior mean and exterior mean
    upts = find(phi<=0);                 % for interior points
    vpts = find(phi>0);                  % for exterior points
    u = sum(I(upts))/(length(upts)+eps); % for interior mean
    v = sum(I(vpts))/(length(vpts)+eps); % for exterior mean
    ...
    %-- intermediate output which is enabled earlier
...
%-- Displays superimposed curve on image
...
function curvature = get_curvature(phi,idx)
    ...
    %-- get central derivatives of SDF at x,y using other indexes
...
function img = im2graydouble(img)    
  ...
  if(isfloat(img)) % checking if image is a double
    if(c==3) 
      img = rgb2gray(uint8(img)); 
    end
  else           % checking if image is a int
    if(c==3) 
      img = rgb2gray(img); 
    end
    img = double(img);
...
%-- level set re-initialization using sussman method
function D = sussman(D, dt)
  % forward/backward differences
  a = D - shiftR(D); %for backward
  b = shiftL(D) - D; % for forward
  c = D - shiftD(D); % for backward
  d = shiftU(D) - D; % for forward
